...
export default function TourApplyModal() {
  const { open, selectedIndex, closeTourApplyModal } = useTourApplyModal()
  const [tourIndex, setTourIndex] = useState(selectedIndex)
  const [form, setForm] = useState(EMPTY_FORM)
  const [loading, setLoading] = useState(false)
  const [submitted, setSubmitted] = useState(false)
  const [error, setError] = useState<string | null>(null)

  // Sync the selected tour whenever the modal is opened
  useEffect(() => {
    if (open) {
  ...
  const tour = UPCOMING_TOURS[tourIndex] ?? UPCOMING_TOURS[0]
  ...
  const handleTourChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    setTourIndex(Number(e.target.value))
  }
      ...
      <DialogTitle sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', pb: 1 }}>
        <Box>
          ...
          <Typography variant="caption" sx={{ color: 'rgba(255,255,255,0.45)' }}>
            {tour.location} · {tour.dates || tour.year}
          </Typography>
        </Box>
        ...
        {submitted ? (
          <Box sx={{ py: 4, textAlign: 'center' }}>
            ...
            <Typography sx={{ color: 'rgba(255,255,255,0.6)' }}>
              Thanks! We'll be in touch within 24 hours with more details about the tour.
            </Typography>
          </Box>
        ) : (
          <Grid container spacing={2} sx={{ pt: 1 }}>
            <Grid size={{ xs: 12 }}>
              <TextField fullWidth select label="Tour" name="tour" value={tourIndex} onChange={handleTourChange} size="small" sx={inputSx}
                slotProps={{ select: { MenuProps: { slotProps: { paper: { sx: { bgcolor: '#021a4a', color: '#fff' } } } } } }}
              >
                {UPCOMING_TOURS.map((t, i) => (
                  <MenuItem key={`${t.name}-${t.year}`} value={i}>{t.name} ({t.year})</MenuItem>
                ))}
              </TextField>
            </Grid>
